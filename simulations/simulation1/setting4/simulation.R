...
nrep <- 200
p1.vec <- seq(from=0.05, to=0.5,by=0.05)
n <- 1000
np <- length(p1.vec)
...
for(i in 1:np)
{
  ...
  { 
    ...
    ep <- rbinom(n, 1, p1)
    muy <- ep*2 + rnorm(n, mean=0, sd=0.5)
    y <-  muy + rnorm(n, mean=0, sd=1)
    s <- ep*rbinom(n, 1, 0.9) + (1-ep)*rbinom(n, 1, 0.05)
    
    
    NM.delta <- y
    TFOR.delta <- TFOR.simul1.func(y, 1, p1)
    
    TFKER.delta <- TFKER.func_2(y, 1)
    
    TFSIOR.delta <- TFSIOR.simul1.func(y, s, 1, p1, 0.9, 0.05)
